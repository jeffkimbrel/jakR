...

{
  ...
  for (i in classes) {
    ...
    if (length(fts_results$timeIntervals) > 1) {
      ...
      pred0 = predict(mod, newdata = group0, include = include,
                      se = TRUE)
      pred1 = predict(mod, newdata = group1, include = include,
                      se = TRUE)
      df.group0 = data.frame(T = timePoints, GROUP = group0$class,
                             FIT = pred0$fit, SE = pred0$se)
      df.group1 = data.frame(T = timePoints, GROUP = group1$class,
                             FIT = pred1$fit, SE = pred1$se)
      df_a = data.frame()
      df_a = rbind(df.group0, df.group1) %>% mutate(F = i)
      dat$F = i
